...
from .serializer import (SignUpAdminSerializer, UpdateUserSerializer, VerifyOtpSerializer, PasswordSerializer,
                         UserSerializer, SignUpUserSerializer, NurseSerializer, DoctorSerializer,
                         AddNurseSerializer, AddPatient, UsersName, ResetPasswordSerializer,
                         PatientSerializer, PatientDoctorsSerializer, PatientNurseSerializer)
from .models import (Admin, Doctor, Nurse, Patient)
    ...
    def get(self, request, pk=None):
        ...
        if user.is_superuser == True:
            queryset = self.filter_queryset(
                queryset=User.objects.filter(added_by=user))
            serializer = UserSerializer(queryset, many=True)
            return Response({"results": queryset.count(), "users_data": serializer.data}, status=status.HTTP_200_OK)
...
class NurseUser(generics.ListCreateAPIView):
    permission_classes = [IsAuthenticated]
...
class DoctorUser(generics.ListCreateAPIView):
    permission_classes = [IsAuthenticated]
...
class PatientDetailsAPI(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get(self, request, pk=None):
        ...
        return Response(data={"patient": data}, status=status.HTTP_200_OK)
...
class CountStaffs(generics.ListCreateAPIView):

    def get(self, request):
        doctor = User.objects.filter(role='doctor')
        nurse = User.objects.filter(role='nurse')
        patient = Patient.objects.all()
        return Response({"doctors": len(doctor), "nurses": len(nurse), "patients": len(patient)})
